...
public class DeepLMenuTranslationService implements MenuTranslationService {
    ...
    public DeepLMenuTranslationService(TranslationProperties properties) {
    ...
    public List<String> translate(List<String> texts, String targetLanguage) {
        if (!properties.isEnabled()) {
            throw new MenuTranslationException("translation_disabled",
        ...
        clientOptions.setTimeout(properties.getTimeout());
        clientOptions.setMaxRetries(1);
        clientOptions.setAppInfo("angolo-di-vino", "0.1.0");
        DeepLClient client = new DeepLClient(properties.getAuthKey(), clientOptions);
        ...
        try {
            List<TextResult> results = client.translateText(
                    texts, properties.getSourceLanguage(), targetLanguage, translationOptions);
            ...
            throw new MenuTranslationException("translation_interrupted",
                    "La richiesta di traduzione è stata interrotta.", e);
        }
    }
}
